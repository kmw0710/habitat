...
class TaskBuilder extends Component {
  ...
  saveTask() {
    ...
    if (
      ...
      axios.post('https://naturalhabitat.herokuapp.com/newTask', {numberRecurrences, title, description, startTime, markerID, endTime, category, frequency, userID})
        .then(res => {
          this.props.navigation.goBack();
        })
        .catch((err) => console.error('taskbuilderjs. line 82', err))
    } else {
      axios.put('https://naturalhabitat.herokuapp.com/editTask', {numberRecurrences, taskID, title, description, startTime, endTime, markerID, category, frequency, userID})
      .then((res) => {
